...
struct scale {
  ...
  */
  ...
  static std::vector<T> genScale(double start, double stop, long N, scaleTypeT sct, valLocationT ltype)
  {
  ...
  static std::vector<T> genScaleDelta(double start, double stop, long N, scaleTypeT sct, valLocationT ltype)
  {
    std::vector<double> scale=genScale(start,stop,N,sct,valLocationV::VERTEX);
    long i,j;
    ...
    if (ltype==valLocationV::CELL) {
      ...
      {
	std::vector<T> tmp=genScale(start,stop,N,sct,valLocationV::CELL);
	scale.front()-=start;
