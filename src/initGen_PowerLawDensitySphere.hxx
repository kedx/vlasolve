#ifndef __INIT_GEN_POWERLAW_DENSITY_SPHERE_HXX__
#define __INIT_GEN_POWERLAW_DENSITY_SPHERE_HXX__
...
class PowerLawDensitySphere : public initGen<gridType> {
...
public:
  ...
  struct PLDS_params {
    ...
    double eta[MAX_SPECIES];
    double n[MAX_SPECIES];
    double smoothingRadius[MAX_SPECIES];
  };

  typedef PLDS_params paramT; 
...
public:
  ...
  void setupParameters(const paramsParser &parser,const solverParamT &sp_)
  {
    ...
    for (i=0;i<sp.nSpecies;i++)
      {
	p.M[i]=1;
	p.alpha[i]=2; // anisotropy
	p.eta[i]=0.1; // eta=1 -> equilibrium (virial theorem satisfied)
	p.n[i]=1; // power law	
	...
	p.eta[i] = parser.get<>("eta",baseT::parserCategory(),p.eta[i],i);
	p.n[i] = parser.get<>("n",baseT::parserCategory(),p.n[i],i);
	...
	rho0[i]=((3.0-p.n[i])*p.M[i])/(4*M_PI*pow(p.R0[i],3.0-p.n[i]));
	sigO2[i]=(3.0-p.n[i])/(5.0-2*p.n[i])*(p.M[i]*p.eta[i])/(p.R0[i]*(p.alpha[i]*0.5+1));
	sigR2[i]=sigO2[i]*0.5*p.alpha[i];

	//printf("sigR2[i]=%e sigO2[i]=%e\n",sigR2[i],sigO2[i]);

	
	Tc[i]=M_PI*sqrt(pow(p.R0[i],3)/(8*sp.G*p.M[i]));
   
	// alpha=-2T/W
	// cf henon(64) / fujiwara (83) 
	// -->  1/(2*sig0*sig0) =5R/(2alpha)
	// for a=0.5 :
	C0[i]=rho0[i]/(sqrt(2*sigR2[i]*M_PI*M_PI*M_PI)*sigO2[i]);
	C1[i]=1.0/(2*sigR2[i]);
	C2[i]=1.0/(sigO2[i]);
      }
  }
  ...
  gridParamT defaultGridParams() {
    ...
    for (int i=0;i<sp.nSpecies;i++)
      {
	double sig2=sigO2[i]+2*sigR2[i];
	if (sigma2<sig2) sigma2=sig2;
      }


    gp.Pmin[0]=0.01;//*0+0.0937081;//0.2/10;
    gp.Pmax[0]=200;//15.9*0+16.9676;//20;
   
    //printf("sigmaR=%g sigmaO=%g --> %g=>%g\n",sqrt(sigR2[0]),sqrt(sigO2[0]),sqrt(sigO2[0]+2*sigR2[0]),15*sqrt(sigma2));

    gp.Umin[0]=-16*sqrt(sigma2);
    gp.Umax[0]=16*sqrt(sigma2);
    gp.Umin[1]=0;
    gp.Umax[1]=1.6;
...
private:
  ...
  double C0[MAX_SPECIES];
  double C1[MAX_SPECIES];
  double C2[MAX_SPECIES];
  double sigR2[MAX_SPECIES];
  double sigO2[MAX_SPECIES];
...
private:
  ...
  */
  ...
  {
    double sm=0.5*(1.+erf((p.R0[s]-r)/p.smoothingRadius[s]));
    double res=sm*C0[s]*exp(-u*u*C1[s]-((j*j)/(r*r))*C2[s])*pow(r,-p.n[s]);

      //C1[s]*exp(-(u*u+(j*j)/(r*r))/C2[s]);
...
public:
  ...
  virtual double query(const queryT &what, double param=0)
  {
	...
	return kernelMass(param);
      default:
	printf("WARNING in initGen_PLDF::query : invalid query (%d)!\n",(int)what);
	return -1;
      }
  }

  PowerLawDensitySphere():baseT() {
  }

  ~PowerLawDensitySphere() {
  }
