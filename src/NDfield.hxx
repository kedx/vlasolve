#ifndef __ND_FIELD_HXX__
#define __ND_FIELD_HXX__
...
int Save_NDfield(NDfield *field,const char *filename)
{
  ...
  for (i=1;i<field->ndims;i++) sprintf(dims_msg,"%s,%d",dims_msg,field->dims[i]);
  ...
  printf ("Saving %s %s to file %s ...",dims_msg,msg,filename);fflush(0);
  fflush(0);
...
int LoadDensity_CIC(const char *fname,OLD_density_grid *density)
{
    ...
    ret=fread(&i,sizeof(int),1,f);
    myIO::fread(&(density->Nx),sizeof(int),1,f,swap);
    myIO::fread(&(density->Ny),sizeof(int),1,f,swap);
    myIO::fread(&(density->Nz),sizeof(int),1,f,swap);
    myIO::fread(&(density->NNodes),sizeof(int),1,f,swap);

    myIO::fread(&(density->x0),sizeof(float),1,f,swap);
    myIO::fread(&(density->y0),sizeof(float),1,f,swap);
    myIO::fread(&(density->z0),sizeof(float),1,f,swap);
    
    myIO::fread(&(density->dx),sizeof(float),1,f,swap);
    myIO::fread(&(density->dy),sizeof(float),1,f,swap);
    myIO::fread(&(density->dz),sizeof(float),1,f,swap);
    ret=fread(&i,sizeof(int),1,f);
    ...
    for (i=0;i<density->Nx;i++)
      {
	myIO::fread(&(density->grid[offs]),sizeof(FLOAT),density->NNodes/density->Nx,f,swap);
    ...
    i=sizeof(float);
    ret= fread(&i,sizeof(int),1,f);
    myIO::fread(&density->redshift,sizeof(float),1,f,swap);
    myIO::fread(&density->smoothing,sizeof(float),1,f,swap);
    ret=fread(&i,sizeof(int),1,f);
...
int Save_NDfieldPartial(char *filename, NDfield *header, NDfield *field, int periodic)
{
  ...
  for (i=0;i<header->ndims;i++) x0[i]=coordi[i]=(long)(header->dims[i]*(coord[i]-header->x0[i])/header->delta[i]);
  ...
  for (j=0;j<n*elSize;j+=elSize)
	...
	if (!k) fwrite((char*)field->val + j,elSize,1,f);
...
NDfield *Load_NDfieldHeader(const char *filename)
{
  ...
  f=fopen(filename,"r");
  myIO::fread(&i,sizeof(int),1,f,swap);
  myIO::fread(tag,sizeof(char),16,f,swap);
  myIO::fread(&i,sizeof(int),1,f,swap);
  tag[15]='\0';
  ...
  if (strcmp(tag,NDFIELD_TAG))
	...
	myIO::fread(&i,sizeof(int),1,f,swap);
      ...
      if (i==44)
	{
	  myIO::fread(dims,sizeof(int),3,f,swap);
	  myIO::fread(tmpf,sizeof(float),8,f,swap);
	  myIO::fread(&i,sizeof(int),1,f,swap);
  ...
  else
    {
      ...
      myIO::fread(dims,sizeof(int),NDFIELD_MAX_DIMS,f,swap);
      myIO::fread(&fdims_index,sizeof(int),1,f,swap);
      myIO::fread(&datatype,sizeof(int),1,f,swap);
      myIO::fread(x0,sizeof(double),NDFIELD_MAX_DIMS,f,swap);
      myIO::fread(delta,sizeof(double),NDFIELD_MAX_DIMS,f,swap);
      myIO::fread(dummy,sizeof(char),160,f,swap);
      myIO::fread(&i,sizeof(int),1,f,swap);
    }
...
NDfield *Load_NDfieldChunk(char *filename, double *x0, double *delta, int periodic)
{
  ...
  if (strcmp(tag,NDFIELD_TAG))
  ...
  else
    {
      myIO::fread(&i,sizeof(int),1,f,swap);
      myIO::fread(comment,sizeof(char),80,f,swap);
      myIO::fread(&lndims,sizeof(int),1,f,swap);
      myIO::fread(ldims,sizeof(int),NDFIELD_MAX_DIMS,f,swap);
      myIO::fread(&fdims_index,sizeof(int),1,f,swap);
      myIO::fread(&datatype,sizeof(int),1,f,swap);
      myIO::fread(lx0,sizeof(double),NDFIELD_MAX_DIMS,f,swap);
      myIO::fread(ldelta,sizeof(double),NDFIELD_MAX_DIMS,f,swap);
      myIO::fread(dummy,sizeof(char),160,f,swap);
      myIO::fread(&i,sizeof(int),1,f,swap);
      myIO::fread(&i,sizeof(int),1,f,swap);
    }
  ...
  */
  ...
  for(n=0;n<tmpi*elSize;n+=elSize)
      ...
      if (isgrafic)
	{
      ...
      myIO::fread((char*)field->val + n,elSize,1,f,swap);
      i=0;coord[i]++;
      if (*coord>=*imax)
...
int IsNDfield(const char *filename)
{
  ...
  fclose(f); 
  tag[15]='\0';
  if (strcmp(tag,NDFIELD_TAG)) return 0;
...
NDfield *Load_NDfieldFromFITS(const char *filename)
{
  ...
  return field;
#endif
  fprintf(stderr,"ERROR: to load '%s', CfitsIO library is needed.\n",filename);
...
NDfield *Load_NDfield(const char *filename)
{
  ...
  myIO::fread(&i,sizeof(int),1,f,swap);
  myIO::fread(field->val,sizeof_NDfield(field->datatype),field->nval,f,swap);
  myIO::fread(&i,sizeof(int),1,f,swap);
